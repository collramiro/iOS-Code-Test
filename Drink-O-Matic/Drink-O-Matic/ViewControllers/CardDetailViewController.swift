...
class CardDetailViewController: StatusBarAnimatableViewController, UIScrollViewDelegate {
    ...
    var drinkViewModel: Drink! {
        didSet {
            if self.cardContentView != nil {
                self.cardContentView.drinkModel = drinkViewModel
            }
        }
    }
    ...
    var drinkDetailModel: DrinkDetails! {
        didSet {
            if self.cardContentView != nil {
                self.cardContentView.drinkDetailModel = drinkDetailModel
            }
        }
    }

    var unhighlightedDrinkViewModel: Drink!
    ...
    override func viewDidLoad() {
        ...
        cardContentView.drinkModel = drinkViewModel
        cardContentView.drinkDetailModel = drinkDetailModel
    ...
    func setTitleAndDescription() {
        ...
        //set drink description and ingredients
        let descriptionTitleAttributes: [NSAttributedString.Key: Any] = [
            .font: UIFont.systemFont(ofSize: 14, weight: .bold),
            .foregroundColor: UIColor.darkText]
        
        let descriptionBodyAttributes: [NSAttributedString.Key: Any] = [
            .font: UIFont.systemFont(ofSize: 14, weight: .light),
            .foregroundColor: UIColor.darkText]
        
        let ingredientsAttributedString = NSMutableAttributedString()
        
        if let ingredientsToShow = drinkDetailModel?.ingredients {
            ingredientsAttributedString.append(NSAttributedString(string: "Ingredients\n", attributes: descriptionTitleAttributes))
            ...
            for ingredient in ingredientsToShow {
                ...
                ingredientsAttributedString.append(NSAttributedString(string: ingredientString, attributes: descriptionBodyAttributes))
            }
        }
        
        ingredientsAttributedString.append(NSAttributedString(string: "\n\nHow to prepare", attributes: descriptionTitleAttributes))
        
        if let instructions = drinkDetailModel.instructions, !instructions.isEmpty {
            ingredientsAttributedString.append(NSAttributedString(string: "\n\n" + instructions, attributes: descriptionBodyAttributes))
        } else {
            ingredientsAttributedString.append(NSAttributedString(string: "\n\nNo description available", attributes: descriptionBodyAttributes))
        }
        
        descriptionLabel.attributedText = ingredientsAttributedString
    }

    func didSuccessfullyDragDownToDismiss() {
        drinkViewModel = unhighlightedDrinkViewModel
